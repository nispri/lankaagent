...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self: "RateLimitMiddleware", app, requests: int = 100, window: int = 60) -> None:
        super().__init__(app)
        self.requests = requests
        self.window = window
        self._buckets: dict[str, list[float]] = {}

    async def dispatch(self: "RateLimitMiddleware", request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self: "LoggingMiddleware", request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        import structlog
